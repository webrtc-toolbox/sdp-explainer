...
import type {Record, SessionDescription, Extmap, RTPMap, RTCPFeedback, Fmtp} from 'sdp-parser'
import './index.css'
...
function ExplainExtMapAllowMixed() {
  return <div>
   ...
   capable of supporting this new mode.`}</p>
    <p>
      <a href="https://www.rfc-editor.org/rfc/rfc8285.html#section-6" target='_blank'>RFC 8285</a>
    </p>
  </div>
}
...
function ExplainIceUfrag() {
  return <div>
   ...
   they MUST have identical ice-pwd's`}</p>
    <p>
      <a href="https://datatracker.ietf.org/doc/html/draft-ietf-mmusic-ice-sip-sdp-24" target='_blank'>Session
        Description Protocol (SDP) Offer/Answer procedures for Interactive Connectivity Establishment (ICE)</a>
    </p>
  </div>
}
...
function ExplainConnection() {
  return <div>
    <p style={{'fontWeight': 700}}>{` c=<nettype> <addrtype> <connection-address>`}</p>
...
function ExplainRTCP() {
  return <div>
    <p
      style={{'fontWeight': 700}}>{`rtcp-attribute = "a=rtcp:" port  [nettype space addrtype space connection-address] CRLF`}</p>
...
function ExplainRecvOnly() {
  return Marked(`If the offerer wishes to only receive media from its peer, it MUST mark the stream as recvonly.`);
}
